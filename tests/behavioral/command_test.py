...
@pytest.mark.parametrize("add_children,remove_children,exception_during_removal",
                         [(1, 1, nullcontext()),
                          (10, 10, nullcontext()),
                          (1, 2, pytest.raises(IndexError))
                          ])
def test_composite_command(add_children: int,
                           remove_children: int,
                           exception_during_removal: AbstractContextManager) -> None:
