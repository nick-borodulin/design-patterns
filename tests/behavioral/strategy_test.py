...
def _generate_expected_output(
    input_list: list[T], strategy_class: Type[RemoveHead | RemoveTail]
) -> list[T]:
    expected_output = []
    if input_list:
...
@pytest.mark.parametrize("input_list", [[1, 2, 3], ["one", "two"], []])
...
def test_remove_head(
    ...
    expected_output = _generate_expected_output(
        input_list=input_list, strategy_class=strategy_class
    )
